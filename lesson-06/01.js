...
function updateImage() {
  ...
  imgElement.src = WEB_TECH_IMAGES[currentImageIndex];
}


// Button NEXT
document.getElementById('next-button').addEventListener('click', function () {
  currentImageIndex = (currentImageIndex + 1) % WEB_TECH_IMAGES.length;                            // Loop: if index = 3; 3 % 3(length) = 0
  updateImage();
})
...
document.getElementById('prev-button').addEventListener('click', function () {
  currentImageIndex = (currentImageIndex - 1 + WEB_TECH_IMAGES.length) % WEB_TECH_IMAGES.length;   // Loop: if index = 0; (0 - 1 + 3) % 3 = 2
  updateImage();
})
